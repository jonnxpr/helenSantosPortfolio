...
- Keep all writes inside the owning repository.
- Never let hooks, scripts, or follow-up actions rewrite home-dir configs or unrelated governance files.
- `update-agent-context.ps1` may update repo-local `.specify/context/*.md` files only.
...
3. **Execute plan workflow**: Follow the structure in IMPL_PLAN template to:
   ...
   - Phase 1: Update repo-local agent context by running the agent script
